...
function createWindow() {
    ...
    setInterval(() => {
        ...
        // Send idle status to all windows
        win.webContents.send('idle-status-changed', {
            isIdle: idleTime >= IDLE_THRESHOLD,
            idleTime
        })
    }, 1000)
    ...
    powerMonitor.on('lock-screen', () => {
        win?.webContents.send('idle-status-changed', { isIdle: true, reason: 'locked' })
    })

    powerMonitor.on('unlock-screen', () => {
        // Immediate check will correct the state on next tick,
